...
def test_stats_degrades_to_zero_when_sources_error() -> None:
    # A locked DB or broken index must not turn the public home page's
    # stats fetch into a 500 — counts degrade to 0 instead.
    broken_db = SimpleNamespace(list_users=lambda: (_ for _ in ()).throw(RuntimeError("locked")))
    broken_index = SimpleNamespace(get_snapshot=lambda: (_ for _ in ()).throw(RuntimeError("boom")))
    app = HomePageAPI(_passthrough, database=broken_db, library_index=broken_index)

    status, data = _get_json(app, "/api/stats")
    assert status.startswith("200")
