...
- **`matched_page_count` (2026-08-29 addendum).** A new optional per-entry field: how many of `page_count` pages the volume's `.mokuro` references actually exist inside the `.cbz`. Producer-computed, never accepted from a PUT (same rule as `archive_size` and the freshness stamps). It sits directly AFTER `page_count`, not with the file-stat group — it qualifies that number and the two are only legible as a pair. `page_count - matched_page_count` is the missing-page count; a reader must clamp at zero, since two pages naming the same image both match and a duplicated `img_path` can overmatch.
  - **Matching is a port of the client's `matchImagesToPages`** (`src/lib/import/processing.ts`), not an independent implementation: exact normalized path, then stem (an extension changed since OCR ran), then the whole-volume positional fallback when the leftovers line up one-to-one and under half the pages matched by name. The archive's file list is filtered the way the client's own unpacker filters it (`isSystemFile`, `isImageExtension`, and the volume's embedded `<Volume>.webp` cover excluded) — matching against a set of files the reader would never load would report a page as present that the reader cannot show. `count_matched_pages` in `reader_compat.py` deliberately does NOT port the fallback's `naturalSort` pairing: a count cannot observe the order. Parity was verified by running the client's function under vitest over the same 14 cases.
  - **Omitted, never zeroed, when undetermined.** No key at all when the archive could not be opened (an OSError — a permissions problem or a vanished mount is not damage) or when the sidecar's `pages` array names no images at all. `0` means "checked, found nothing", which is real damage, and the two must not collide. A `BadZipFile`/truncated archive IS damage and reports `0`.
  - **An unreadable archive is not cached.** `_compile_volume` returns a `cacheable` flag alongside the entry; an OSError-on-open volume writes no `series_entry_cache` row, so the next pass asks the filesystem again instead of serving the shrug until the file happens to change. Everything else caches as before, and the new key's REQUIRED-key access in `_entry_from_dict` is the backfill: a row written before this field existed raises `KeyError`, misses, and is recompiled.
  - **`catalog_series` carries the series-level totals** (`missing_pages`, `damaged_volumes`), added by `ALTER TABLE` with a `0` default for existing databases and rewritten wholesale by every pass. The catalog's "Missing pages" filter keys off `damaged_volumes`, not the page total: one volume short by 200 pages and 200 volumes short by one are the same number and very different libraries.
